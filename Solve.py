...
x = np.arange(self.DX[0], self.DX[1], h) 
count = len(x) - 1 
xf,yf,zf = self.Runge_Kutta(sys,iCond) 
y = yf[:4] 
z = zf[:4] 
for i in range(3,count): 
currentZ =
z[i]+h/24*(55*f(x[i],y[i],z[i])-59*f(x[i-1],y[i-1],z[i-1])+37*f(x[i-2],y[i-2],z[i-2])-9*f(x[i-3],y[i-3],z[i-3])) 
currentY = y[i]+h/24*(55*g(x[i],y[i],z[i])-59*g(x[i-1],y[i-1],z[i-1])+37*g(x[i-2],y[i-2],z[i-2])-9*g(x[i-3],y[i-3],z[i-3])) 
y.append(currentY) 
z.append(currentZ) 
return x,
'''